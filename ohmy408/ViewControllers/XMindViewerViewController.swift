...
import SnapKit
import Foundation
// import ZIPFoundation // 临时禁用，待SPM修复
...
class XMindViewerViewController: UIViewController {
    ...
    private func setupConstraints(errorStackView: UIStackView) {
        ...
        loadingIndicator.snp.makeConstraints { make in
            ...
            make.centerY.equalTo(loadingView).offset(-30)
        }
        ...
        loadingLabel.snp.makeConstraints { make in
            ...
            make.centerX.equalTo(loadingView)
            make.leading.greaterThanOrEqualTo(loadingView).offset(20)
            make.trailing.lessThanOrEqualTo(loadingView).offset(-20)
        }
        ...
        errorStackView.snp.makeConstraints { make in
            ...
            make.leading.greaterThanOrEqualTo(errorView).offset(40)
            make.trailing.lessThanOrEqualTo(errorView).offset(-40)
        }
    ...
    private func setupNavigationBar() {
        ...
        // 使用统一的按钮风格创建导航按钮
        let closeButton = UIBarButtonItem.createStandardButton(
            systemName: "xmark",
            target: self,
    ...
    private func parseXMindFile(_ fileURL: URL) throws -> [String: Any] {
        ...
        if !FileManager.default.fileExists(atPath: contentJSONURL.path) {
            print("未找到content.json，尝试解析content.xml")
    ...
    private func unzipXMindFile(from sourceURL: URL, to destinationURL: URL) throws {
        ...
        // 使用ZIPFoundation解压
        do {
            // TODO: 实现ZIP解压缩功能，当前禁用ZIPFoundation
            throw NSError(domain: "XMindViewer", code: 1, userInfo: [NSLocalizedDescriptionKey: "XMind功能需要ZIPFoundation库支持"])
            print("XMind文件解压成功")
    ...
    private func updateThemeButton() {
        // 更新所有导航栏按钮的主题感知样式（包括主题按钮图标更新）
        updateNavigationBarAppearance()
        
        print("🔄 XMindViewerViewController 主题按钮已更新")
    }
    ...
    private func updateNavigationBarAppearance() {
        ...
        // 使用统一的橙色样式更新所有按钮
        navigationItem.leftBarButtonItem?.applyThemeAwareStyle()
