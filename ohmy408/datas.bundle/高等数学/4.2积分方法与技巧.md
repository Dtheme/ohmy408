...
**第二步：代入积分**
$$\int \frac{1}{\sqrt{4 - x^2}} \, dx = \int \frac{1}{2 \cos t} \cdot (2 \cos t \, dt) = \int \frac{2 \cos t \, dt}{2 \cos t} = \int 1dt$$
...
**第三步：化简被积函数**
$$= \int \frac{t^2 \cdot t}{\sqrt{t^2 + 1}} \cdot \left(-\frac{1}{t^2}\right) dt = \int \frac{t^3}{\sqrt{t^2 + 1}} \cdot \left(-\frac{1}{t^2}\right) dt = -\int \frac{t}{\sqrt{t^2 + 1}} \, dt$$
...
- 符号处理：由于积分中 $x^2$ 出现，可假设 $x > 0$，则 $|x| = x$
...
> 
> **根式换元**：
> - 核心是**有理化处理**，将无理函数变为有理函数
> - 关键是正确建立 $x$ 与 $t$ 的反函数关系
> - 微分变换要通过求导得到，不能死记公式
> 
> **倒代换**：
> - 核心是**次数转换**，将高次分母变为低次分子
> - 适用于分母次数远高于分子的情况
> - 要注意符号变化和定义域的对应关系
...
| 积分类型 |  标准解法  | 技巧要点 | 结果 |
|:---------|:---------|:---------|:-----|
| $\int \frac{1}{\sin x} dx$ | 乘除技巧 | 分子分母同乘 $(\csc x + \cot x)$ | $-\ln\|\csc x + \cot x\| + C$ |
| $\int \frac{1}{\cos x} dx$ | 乘除技巧 | 分子分母同乘 $(\sec x + \tan x)$ | $\ln\|\sec x + \tan x\| + C$ |
| $\int \frac{1}{\sin x \cos x} dx$ | 部分分式 | $\frac{1}{\sin x \cos x} = 2(\csc 2x)$ | $\ln\|\tan x\| + C$ |
| $\int \sqrt{a^2 - x^2} \, dx$ | 三角替换 | $x = a \sin t$ | $\frac{a^2}{2}(\arcsin \frac{x}{a} + \frac{x\sqrt{a^2-x^2}}{a^2}) + C$ |
| $\int \sqrt{x^2 + a^2} \, dx$ | 三角替换 | $x = a \tan t$ | $\frac{x\sqrt{x^2+a^2}}{2} + \frac{a^2}{2}\ln\|x + \sqrt{x^2+a^2}\| + C$ |
| $\int \sqrt{x^2 - a^2} \, dx$ | 三角替换 | $x = a \sec t$ | $\frac{x\sqrt{x^2-a^2}}{2} - \frac{a^2}{2}\ln\|x + \sqrt{x^2-a^2}\| + C$ |
| $\int \frac{1}{\sqrt{ax^2 + bx + c}} dx$ | 配方+三角替换 | 先配方再三角替换 | 反三角函数形式 |
| $\int \frac{dx}{x^2 + px + q}$ | 配方+部分分式 | 配方后判别式处理 | 反正切或对数形式 |

**特殊积分类型解法示例**：

1. **余割积分**：$\int \csc x \, dx = \int \frac{1}{\sin x} \, dx$
...
2. **正割积分**：$\int \sec x \, dx = \int \frac{1}{\cos x} \, dx$
...
3. **正弦余弦乘积积分**：$\int \frac{1}{\sin x \cos x} \, dx$
...
7. **配方积分**：$\int \frac{1}{\sqrt{x^2 + 2x + 5}} \, dx$
...
8. **有理函数配方积分**：$\int \frac{1}{x^2 + px + q} \, dx$
...
**递推公式**：$$I_n = \frac{x(1 + x^2)^n + 2nI_{n-1}}{2n + 1}$$
...
> **例题4.1**：计算 $\int \frac{1}{x^2 - 1} \, dx$

**解答**：
$$\frac{1}{x^2 - 1} = \frac{1}{(x-1)(x+1)} = \frac{A}{x-1} + \frac{B}{x+1}$$
通分得：$1 = A(x+1) + B(x-1)$
令 $x = 1$：$1 = 2A$，得 $A = \frac{1}{2}$
令 $x = -1$：$1 = -2B$，得 $B = -\frac{1}{2}$
...
对于不可分解的二次因式 $ax^2 + bx + c$（$b^2 - 4ac < 0$），对应项为：
$$\frac{Ax + B}{ax^2 + bx + c}$$

> **例题4.3**：计算 $\int \frac{1}{x^3 + 1} \, dx$

**解答**：
$$x^3 + 1 = (x+1)(x^2-x+1)$$
$$\frac{1}{x^3 + 1} = \frac{A}{x+1} + \frac{Bx + C}{x^2 - x + 1}$$
通分后比较系数，得：$A = \frac{1}{3}$，$B = -\frac{1}{3}$，$C = \frac{2}{3}$
$$\int \frac{1}{x^3 + 1} \, dx = \frac{1}{3} \int \frac{1}{x+1} \, dx + \int \frac{-\frac{1}{3}x + \frac{2}{3}}{x^2 - x + 1} \, dx$$
...
本节按照难度分为四个层次，每个层次都有明确的学习目标和解题要求：
...
**学习策略**：建议逐层练习，每层练习完成后再进入下一层。

---

#### 7.4.1 基础练习解答（⭐⭐）

**练习目标**：熟练掌握基本积分方法的标准应用
...
回代得到复杂表达式，最终结果为：
$$\int \frac{x^4}{(x^2 + 1)^3} \, dx = \frac{x^3}{3(x^2 + 1)} - \frac{x}{2(x^2 + 1)} + \frac{3\arctan x}{8} + C$$
...
**专题说明**：本节收录一些特殊类型的积分问题，包括变上限积分、对称性积分等，这些题型在考试中经常出现。
...
**关键方法：利用对称性**
我们考虑两个积分的和：
$$I + \int_0^1 (1-x) f(1-x) dx = \int_0^1 [x + (1-x)] f(1-x) dx = \int_0^1 f(1-x) dx$$

对 $\int_0^1 f(1-x) dx$ 进行变量替换 $u = 1-x$：
$$\int_0^1 f(1-x) dx = \int_1^0 f(u) (-du) = \int_0^1 f(u) du = A$$

因此：
$$I + \int_0^1 (1-x) f(1-x) dx = A \quad \cdots (1)$$

对于 $\int_0^1 (1-x) f(1-x) dx$，令 $t = 1-x$：
$$\int_0^1 (1-x) f(1-x) dx = \int_1^0 t f(t) (-dt) = \int_0^1 t f(t) dt$$

但是，对 $\int_0^1 t f(t) dt$ 再次使用变量替换 $s = 1-t$：
$$\int_0^1 t f(t) dt = \int_1^0 (1-s) f(1-s) (-ds) = \int_0^1 (1-s) f(1-s) ds$$

这个积分正是我们要求的 $I$！因此：
$$\int_0^1 (1-x) f(1-x) dx = I$$
